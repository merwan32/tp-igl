...
<body class="g-sidenav-show bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        {% else %}
          <div class="card card-body border-0 shadow table-wrapper table-responsive mx-5 p-0 admin">  
...
<script data-position="{{ positions }}">

  const data = document.currentScript.dataset;
  const positions = JSON.parse(data.position);
    ...
    var map = new H.Map(
        ...
        {
          zoom: 7,
          center: { lat: 36.210596, lng: 2.917836 }
      });
      
      for(i=0; i< positions.length;i++){
        var Marker = new H.map.Marker({ lat: positions[i].lat, lng: positions[i].long });
